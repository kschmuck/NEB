...
def energy_and_gradient(xyzs):
    ...
    in_file.add(geo)
    in_file.add(rem)
    in_file.run(name="ethane")
    out_file = qc.read("ethane.out", silent=True)
    gradient = out_file.opt.gradient_vector[0].T.flatten()
    energy = out_file.opt.energies[0]
    return energy, gradient # out_file.opt.energies[0], out_file.opt.gradient_vector[0]
